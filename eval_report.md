...
What's misleading about the headline accuracy number: it is dominated by the out_of_scope majority class (~88% of real traffic per taxonomy.yaml) -- a baseline that always predicts out_of_scope can beat a real classifier on accuracy while scoring near-zero on macro-F1. Macro-F1 and per-intent F1 are the numbers that actually reflect classification quality here.
